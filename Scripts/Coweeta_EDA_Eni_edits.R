...
coweeta.species.update = coweeta.species[,-c(1)] %>% 
  t(.) %>%   
  mutate(sum.ba = sum(V1:V50))

coweeta.species.BA = coweeta.species %>% 
  summary(meano3 = mean(Ozone))

ggplot() +
  geom_freqpoly(data = coweeta.species, mapping = aes(y = ACRU))
  geom_point(data = coweeta.species, mapping = aes(x = Plot, y = ACRU))
  geom_bar(data = coweeta.species, )
